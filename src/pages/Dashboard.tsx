    ...
    useEffect(() => {
        ...
        setActionButton(
            <button
                ...
                className="flex items-center gap-2 bg-accent-600 text-white px-4 py-2 rounded-lg hover:bg-accent-700 hover:shadow-lg hover:shadow-accent-500/40 transition-all shadow-sm"
            >
        ...
        <div className="p-8 max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {projects.map((project) => (
                    <div key={project.id} className="bg-white dark:bg-neutral-800 rounded-xl border border-gray-200 dark:border-neutral-700 shadow-sm hover:shadow-md hover:shadow-accent-500/10 dark:hover:shadow-accent-500/20 transition-all group neon-card-hover relative">
                        <Link to={`/project/${project.id}`} className="block p-6">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-accent-50 dark:bg-accent-900/30 text-accent-600 dark:text-accent-400 rounded-lg">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 rounded-full hover:bg-gray-100 dark:hover:bg-neutral-700 transition-opacity"
                                    >
                                        <MoreVertical className="w-5 h-5" />
                                    </button>
                                    {openDropdownId === project.id && (
                                        <div
                                            className="absolute right-0 top-8 bg-white dark:bg-neutral-800 border border-gray-100 dark:border-neutral-700 shadow-lg rounded-lg p-1 z-10 min-w-[120px]"
                                        >
                                            <button
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                ...
                                                className="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-neutral-700 rounded flex items-center gap-2"
                                            >
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2 group-hover:text-accent-600 dark:group-hover:text-accent-400 transition-colors">{project.name}</h3>
                            <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-2 mb-4 h-10">{project.description}</p>

                            <div className="flex items-center justify-between text-sm text-gray-400 dark:text-gray-500 pt-4 border-t border-gray-100 dark:border-neutral-700">
                                <div className="flex items-center gap-1">
                                    <Calendar className="w-4 h-4" />
                                    <span>{new Date(project.createdAt).toLocaleDateString('fr-FR')}</span>
                                </div>
                                <div className="bg-gray-100 dark:bg-neutral-700 px-2 py-1 rounded text-gray-600 dark:text-gray-300 font-medium">
                ...
                {projects.length === 0 && (
                    <div className="col-span-full text-center py-12 bg-white dark:bg-neutral-800 rounded-xl border border-dashed border-gray-300 dark:border-neutral-600">
                        <div className="mx-auto w-16 h-16 bg-gray-50 dark:bg-neutral-700 rounded-full flex items-center justify-center mb-4">
                        ...
                        <button
                            onClick={openCreateModal}
                            className="inline-flex items-center gap-2 text-accent-600 dark:text-accent-400 hover:text-accent-700 dark:hover:text-accent-300 font-medium"
                        >
                            <Plus className="w-5 h-5" />
                            Créer un Projet
                        </button>
